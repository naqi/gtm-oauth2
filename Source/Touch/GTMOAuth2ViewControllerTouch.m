...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if (self.request && [self.initialHTMLString length] > 0) {
        ...
        if ([[webView.request.URL absoluteString] containsString:@"accounts.google.com"] && self.email) {
            [webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"document.getElementById('Email').value = '%@';", self.email]];
        }
